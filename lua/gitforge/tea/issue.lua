...
function GiteaIssue:convert_cmd_result_to_issue(json_input)
    local json_issue = vim.fn.json_decode(json_input)
    return {
        title = json_issue.title,
        body = json_issue.title,
        author = { login = json_issue.user },
        project = json_issue.project,
        number = json_issue.index,
        createdAt = json_issue.created,
        closed = json_issue.state == "closed",
        closedAt = json_issue.closedAt,
        state = json_issue.state,
        assignees = json_issue.assignees,
        labels = json_issue.labels,
        comments = conv_gitea_comments(json_issue.comments),
    }
end
